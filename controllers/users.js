...
const initialilizing = async (req, res) => {
  try {
    ...
    return responseHandler(res, 200, null, {
      message: "Already generated Super Admin",
    });
  } catch (error) {
...
const signin = async (req, res) => {
  ...
  try {
    ...
    if (user.passwordFlag === false) {
      ...
      return responseHandler(res, 200, null, {
        User_Credentials: `${user._id}`,
        token,
        message: "since it is your default password please change it ASAP",
      });
    } else {
      //res.send({user,token});
      logger.log("info", `200 response at signin `);
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    return responseHandler(res, 400, err, null);
  }
};
